...
class DotsIndicatorDecoration(
    ...
    private val lineLength = 40.dp //todo udjust to 40dp to see the difference
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        val indicatorPosY = parent.height - indicatorHeight / 0.4f//Todo adjust to position indicator

        val activePosition: Int = when (parent.layoutManager) {
            is LoopingLayoutManager -> {
                (parent.layoutManager as LoopingLayoutManager?)!!.findFirstCompletelyVisibleItemPosition()
            }
            else -> {
