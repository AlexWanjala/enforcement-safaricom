...
import kotlinx.android.synthetic.main.activity_plot_details.edAmount
import kotlinx.android.synthetic.main.activity_plot_details.edPhoneNumber
import kotlinx.android.synthetic.main.activity_plot_details.tvSendPayment
import kotlinx.android.synthetic.main.activity_plot_details.tvSendPushDisabled
import kotlinx.android.synthetic.main.activity_plot_details.tv_message
...
class PlotDetails : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tv_message_header.text ="Plot Details (Plot NO. ones Here)"
            ...
            override fun onSuccess(result: String?) {
                ...
                if(response.success){
                    runOnUiThread {
                        ...
                        Const.instance.setPlot(response.data.plot)

                        tvName.text = response.data.plot.ownerNames
                        tvIdentifyNo.text = ""
                        tvPhoneNumber.text = response.data.plot.ownerPhoneNo
                        tvEmail.text = response.data.plot.ownerEmail

                        tvPlotNumber.text = response.data.plot.plotNumber
                        tvSubCounty.text = response.data.plot.subCountyName
                        tvWard.text = response.data.plot.wardName
                        tvPhysicalAddress.text = response.data.plot.plotAddress
                        tvPlotSize.text = response.data.plot.plotSize
                        tvRentArreas.text = response.data.plot.outstandingArrears
                        tvAnnualRate.text = response.data.plot.annualCharges
                        tvOutStanding.text = response.data.plot.outstandingArrears
                    ...
                    runOnUiThread {
                        tv_message_header
                        Toast.makeText(this@PlotDetails,response.message, Toast.LENGTH_LONG).show()
                    }
                }
            }
            override fun onFailure(result: String?) {
                runOnUiThread {
                    Toast.makeText(this@PlotDetails,result, Toast.LENGTH_LONG).show()
                }
            }
        })
    }
    ...
    private fun generateBill (){
        ...
        val formData = listOf(
            ...
            "customer" to "${Const.instance.getPlot().plotNumber}|${Const.instance.getPlot().ownerNames}|${Const.instance.getPlot().id}", //unitNo | phone | id
            "zone" to  Const.instance.getPlot().zone,
            "subCountyID" to  Const.instance.getPlot().subCountyID,
            "subCountyName" to  Const.instance.getPlot().subCountyName,
            "wardID" to  Const.instance.getPlot().wardID,
            "wardName" to Const.instance.getPlot().wardName,
            ...
            "description" to "${Const.instance.getPlot().ownerNames}| ${Const.instance.getPlot().plotNumber} | ${Const.instance.getPlot().ownerPhoneNo}",
            "deviceId" to getDeviceIdNumber(this)
        )
        executeRequest(formData, biller,object : CallBack {
            override fun onSuccess(result: String?) {
                ...
                if(response.success){
                    ...
                    runOnUiThread {  Toast.makeText(this@PlotDetails,response.message, Toast.LENGTH_LONG).show()}
            ...
            override fun onSuccess(result: String?) {
                ...
                if(response.success){
                    runOnUiThread {
                        ...
                        save(this@PlotDetails,"r_headline",response.data.county.headline)
                        save(this@PlotDetails,"r_dateCreated",response.data.receiptDetails.dateCreated)
                        save(this@PlotDetails,"r_source",response.data.receiptDetails.source)
                        save(this@PlotDetails,"r_currency",response.data.receiptDetails.currency)
                        save(this@PlotDetails,"r_item",item)
                        save(this@PlotDetails,"r_ussd",response.data.county.ussd)
                        save(this@PlotDetails,"r_incomeTypeDescription",response.data.receiptDetails.incomeTypeDescription)
                        save(this@PlotDetails,"r_description",descriptions)
                        save(this@PlotDetails,"r_date",response.data.receiptDetails.dateCreated)
                        save(this@PlotDetails,"r_subCountyName",response.data.receiptDetails.subCountyName)
                        save(this@PlotDetails,"r_zone",response.data.receiptDetails.zone)
                        save(this@PlotDetails,"r_names",response.data.receiptDetails.names)
                        save(this@PlotDetails,"r_transactionCode",response.data.receiptDetails.transactionCode)
                        save(this@PlotDetails,"r_payer",response.data.receiptDetails.paidBy)
                        save(this@PlotDetails,"r_payerPhone",response.data.receiptDetails.customerPhoneNumber)
                        save(this@PlotDetails,"r_billNo",response.data.receiptDetails.billNo)
                        save(this@PlotDetails,"r_receiptNo",response.data.receiptDetails.receiptNo)
                        save(this@PlotDetails,"r_receiptAmount",response.data.receiptDetails.receiptAmount)
    ...
    private fun initListeners() {
        ...
        printing?.printingCallback = object : PrintingCallback {
            override fun connectingWithPrinter() {
                Toast.makeText(this@PlotDetails, "Connecting with printer", Toast.LENGTH_SHORT).show()
            }

            override fun printingOrderSentSuccessfully() {
                Toast.makeText(this@PlotDetails, "Order sent to printer", Toast.LENGTH_SHORT).show()
            }

            override fun connectionFailed(error: String) {
                Toast.makeText(this@PlotDetails, "Failed to connect printer", Toast.LENGTH_SHORT).show()
            }

            override fun onError(error: String) {
                Toast.makeText(this@PlotDetails, error, Toast.LENGTH_SHORT).show()
            }

            override fun onMessage(message: String) {
                Toast.makeText(this@PlotDetails, "Message: $message", Toast.LENGTH_SHORT).show()
            }

            override fun disconnected() {
                Toast.makeText(this@PlotDetails, "Disconnected Printer", Toast.LENGTH_SHORT).show()
                finish()
                startActivity(Intent(this@PlotDetails, ReceivePayment::class.java))
            }
    ...
    private fun getSomePrintables() = java.util.ArrayList<Printable>().apply {
        ...
        val dateCreated = getValue(this@PlotDetails,"r_dateCreated")
        val source = getValue(this@PlotDetails,"r_source")
        val currency = getValue(this@PlotDetails,"r_currency")
        val ussd = getValue(this@PlotDetails,"r_ussd")
        val payerPhone = getValue(this@PlotDetails,"r_payerPhone")
        val payer = getValue(this@PlotDetails,"r_payer")
        val receiptNo = getValue(this@PlotDetails,"r_receiptNo")
        val item = getValue(this@PlotDetails,"r_item")
        val transactioncode = getValue(this@PlotDetails,"r_transactionCode")
        val amount = getValue(this@PlotDetails,"r_receiptAmount")
        val ref = getValue(this@PlotDetails,"r_billNo")
        val username = getValue(this@PlotDetails,"username")
        val names = getValue(this@PlotDetails,"r_names")
        val phone = getValue(this@PlotDetails,"payer_phone")
        val incomeTypeDescription = getValue(this@PlotDetails,"r_incomeTypeDescription")?.capitalize()
        val description = getValue(this@PlotDetails,"r_description")
        val zone = getValue(this@PlotDetails,"r_zone")?.toUpperCase()
        val subCounty = getValue(this@PlotDetails,"r_subCountyName")?.toUpperCase()
        ...
        /*
           ...
           val transactioncode = getValue(this@PlotDetails,"transaction_code")
           val amount = getValue(this@PlotDetails,"amount")
           val ref = getValue(this@PlotDetails,"ref")
           val username = getValue(this@PlotDetails,"username")
           val names = getValue(this@PlotDetails,"payer_names")
           val phone = getValue(this@PlotDetails,"payer_phone")
           val incomeTypeDescription = getValue(this@PlotDetails,"incomeTypeDescription")?.capitalize()
           val description = getValue(this@PlotDetails,"description")
           val date = getValue(this@PlotDetails,"date")
        ...
        val humanDate = date
        val zone = getValue(this@PlotDetails,"zone")
